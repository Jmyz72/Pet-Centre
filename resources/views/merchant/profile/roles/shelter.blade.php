            ...
            @foreach($pets as $pet)
                <div class="bg-white rounded-lg shadow p-4 flex flex-col">
                    <a href="{{ route('pets.show', $pet) }}" class="block">
                        @if($pet->photo)
                            <img loading="lazy" src="{{ asset('storage/' . $pet->photo) }}" alt="{{ $pet->name }}" class="w-full h-40 object-cover rounded-md mb-3">
                        @else
                    ...
                    <div class="mt-3">
                        <a href="{{ route('pets.show', $pet) }}" class="inline-block px-3 py-1 rounded border hover:bg-gray-50 text-sm">View details</a>
                    </div>
                </div>
            @endforeach
        </div>
