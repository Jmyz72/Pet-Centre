        ...
        <div class="flex-1 flex flex-col">
            ...
            <div class="p-4 border-b bg-gray-50 dark:bg-gray-800 dark:border-gray-600 flex items-center justify-between shadow-sm">
                <h2 class="text-lg font-bold text-gray-900 dark:text-gray-200">
                    {{-- --- THE FIX IS HERE --- --}}
                    {{-- We now check if the customer was actually found before trying to use it --}}
                    @if($selectedCustomerId && ($customer = $chatPartners->firstWhere('id', $selectedCustomerId)))
                        {{ $customer->name }}
                    @else
            ...
            <div 
                ...
                x-data="{ autoScroll: function() { setTimeout(() => { this.$el.scrollTop = this.$el.scrollHeight; }, 50); } }"
                x-init="autoScroll()"
            ...
            >
                @forelse ($messages as $msg)
                    <div class="flex {{ $msg->sender_id == auth()->id() ? 'justify-end' : 'justify-start' }}">
                        <div class="relative max-w-[70%]">
                            <div class="px-4 py-2 rounded-xl shadow {{ $msg->sender_id == auth()->id() ? 'bg-gray-900 text-white dark:bg-gray-200 dark:text-gray-900' : 'bg-gray-200 text-gray-900 dark:bg-gray-700 dark:text-gray-200' }}">
                                @if($msg->message)<span>{{ $msg->message }}</span>@endif
                                @if ($msg->image_path)
                                    <div class="mt-2">
                                        @if (str_ends_with(strtolower($msg->image_path), '.pdf'))
                                            <a href="{{ Storage::url($msg->image_path) }}" target="_blank" class="flex items-center gap-2 mt-2 px-3 py-1 bg-gray-300 dark:bg-gray-600 rounded hover:bg-gray-400 dark:hover:bg-gray-500 text-sm text-blue-600 dark:text-blue-300">
                                                <x-heroicon-o-document-text class="h-5 w-5"/><span>View PDF</span>
                                            </a>
                                        @else
                ...
                @empty
                    <div class="flex items-center justify-center h-full text-gray-500">No messages in this conversation yet.</div>
                @endforelse
            </div>
            ...
            <div class="p-4 border-t bg-white dark:bg-gray-800 shadow-inner dark:border-gray-700">
                <form wire:submit.prevent="sendMessage" class="flex flex-col">
                    <div class="flex items-center space-x-2">
                        <input type="text" wire:model.defer="data.message" class="flex-1 border rounded-full px-4 py-2 focus:outline-none focus:ring-2 focus:ring-green-400 dark:bg-gray-900 dark:text-gray-200 dark:border-gray-700" placeholder="Type a message..." autocomplete="off">
                        ...
                        <button type="submit" class="bg-green-600 text-white px-6 py-2 rounded-full hover:bg-green-700 disabled:opacity-50">Send</button>
                    </div>
